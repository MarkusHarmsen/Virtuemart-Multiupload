...
<div id="content">
    <form id="virtuemart_multiupload_form" action="." method="post" enctype="multipart/form-data">
        ...
        <div class="fieldset flash">
            <span class="legend" style="display: block; padding: 0pt 3px; width: 60px;">Options</span>
            
            <div style="margin-top: -15px">
                <label for="file_resize_fullimage"><?php echo $VM_LANG->_('VM_FILES_FORM_RESIZE_IMAGE'); ?></label>
                <input type="checkbox" class="inputbox" id="file_resize_fullimage" checked="checked" value="1" onclick="setResizeParam()" />
                
                <div id="fullsizes" style="padding-top: 10px;">&nbsp;&nbsp;&nbsp;
                    <?php echo $VM_LANG->_('PHPSHOP_PRODUCT_FORM_WIDTH');?>: <input id="fullimage_width" type="text" value="500" class="inputbox" onchange="setResizeParam()" />&nbsp;&nbsp;&nbsp;
                    <?php echo $VM_LANG->_('PHPSHOP_PRODUCT_FORM_HEIGHT');?>: <input id="fullimage_height" type="text" value="500" class="inputbox" onchange="setResizeParam()" />
                </div><br /><br />
                
                
                <label for="file_create_thumbnail"><?php echo $VM_LANG->_('PHPSHOP_FILES_FORM_AUTO_THUMBNAIL'); ?></label>
                <input type="checkbox" class="inputbox" id="file_create_thumbnail" checked="checked" value="1" onclick="setResizeParam()" />
                
                <div id="thumbnailsizes" style="padding-top: 10px;">&nbsp;&nbsp;&nbsp;
                    <?php echo $VM_LANG->_('PHPSHOP_PRODUCT_FORM_WIDTH');?>: <input id="thumbimage_width" type="text" value="<?php echo PSHOP_IMG_WIDTH; ?>" class="inputbox" onchange="setResizeParam()" />&nbsp;&nbsp;&nbsp;
                    <?php echo $VM_LANG->_('PHPSHOP_PRODUCT_FORM_HEIGHT');?>: <input id="thumbimage_height" type="text" value="<?php echo PSHOP_IMG_HEIGHT; ?>" class="inputbox" onchange="setResizeParam()" />
                </div>
            </div>
...
//<![CDATA[
  ...
  function setResizeParam() {
    var fullimageResize   = document.getElementById('file_resize_fullimage').checked,
        fullimageWidth    = document.getElementById('fullimage_width').value,
        fullimageHeight   = document.getElementById('fullimage_height').value,
    
        thumbnailResize   = document.getElementById('file_create_thumbnail').checked,
        thumbnailWidth    = document.getElementById('thumbimage_width').value,
        thumbnailHeight   = document.getElementById('thumbimage_height').value;
    
    document.getElementById('fullsizes').style.display = fullimageResize ? 'block' : 'none';
    document.getElementById('thumbnailsizes').style.display = thumbnailResize ? 'block' : 'none';
    
    swfu.addPostParam('file_resize_fullimage',  fullimageResize ? '1' : '0');
    swfu.addPostParam('fullimage_width',        fullimageWidth);
    swfu.addPostParam('fullimage_height',       fullimageHeight);
    
    swfu.addPostParam('file_create_thumbnail',  thumbnailResize ? '1' : '0');
    swfu.addPostParam('thumbimage_width',       thumbnailWidth);
    swfu.addPostParam('thumbimage_height',      thumbnailHeight);
  }
//]]>
</script>
